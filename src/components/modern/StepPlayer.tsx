    ...
    <div className="border-4 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] rounded-2xl overflow-hidden">
      ...
      <div className="bg-black px-4 py-3 flex items-center justify-between gap-2">
        <button
          onClick={prev}
          disabled={step === 0}
          className="px-4 py-2 bg-yellow-300 border-2 border-yellow-300 font-black uppercase text-sm rounded-xl hover:bg-yellow-400 disabled:opacity-40 transition-colors"
        >
          ◀ Prev
        </button>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={togglePlay}
            className={`px-3 py-2 font-black uppercase text-xs border-2 rounded-xl transition-colors ${
              playing
